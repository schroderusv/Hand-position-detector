...
void bmp280_get_data(I2C_Handle *i2c, double *pres, double *temp) {
    ...
    if (I2C_transfer(*i2c, &i2cTransaction)) {
        ...
        uint32_t valitulos_temp_msb = rxBuffer[3] << 12;
        uint32_t valitulos_temp_lsb = rxBuffer[4] << 4;
        uint32_t valitulos_temp_xlsb = rxBuffer[5] >> 4;
        raw_t = valitulos_temp_msb + valitulos_temp_lsb + valitulos_temp_xlsb;
        
        uint32_t valitulos_press_msb = rxBuffer[0] << 12;
        uint32_t valitulos_press_lsb = rxBuffer[1] << 4;
        uint32_t valitulos_press_xlsb = rxBuffer[2] >> 4;
        raw_p = valitulos_press_msb + valitulos_press_lsb + valitulos_press_xlsb;
    	...
    	*pres = bmp280_convert_pres(raw_p);
