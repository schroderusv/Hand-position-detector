...
void mpu9250_get_data(I2C_Handle *i2c, float *ax, float *ay, float *az, float *gx, float *gy, float *gz) {
    ...
    i2cTransaction.readCount = 14;*/
        ...
        uint16_t valitulos_accel_xout_h = rawData[0] << 8;
        uint16_t valitulos_accel_xout_l = rawData[1];
        data[0] = valitulos_accel_xout_h + valitulos_accel_xout_l;
    
        uint16_t valitulos_accel_yout_h = rawData[2] << 8;
        uint16_t valitulos_accel_yout_l = rawData[3];
        data[1]= valitulos_accel_yout_h + valitulos_accel_yout_l;
        
        uint16_t valitulos_accel_zout_h = rawData[4] << 8;
        uint16_t valitulos_accel_zout_l = rawData[5];
        data[2] = valitulos_accel_zout_h + valitulos_accel_zout_l;
        
        uint16_t valitulos_gyro_xout_h = rawData[8] << 8;
        uint16_t valitulos_gyro_xout_l = rawData[9];
        data[4] = valitulos_gyro_xout_h + valitulos_gyro_xout_l;
        
        uint16_t valitulos_gyro_yout_h = rawData[10] << 8;
        uint16_t valitulos_gyro_yout_l = rawData[11];
        data[5] = valitulos_gyro_yout_h + valitulos_gyro_yout_l;
        
        uint16_t valitulos_gyro_zout_h = rawData[12] << 8;
        uint16_t valitulos_gyro_zout_l = rawData[13];
        data[6] = valitulos_gyro_zout_h + valitulos_gyro_zout_l;
